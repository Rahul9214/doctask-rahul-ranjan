...
Phase 09 converts those guarantees into executable suites: prompt-injection source text, insufficient
evidence / no-bluffing, provenance/tamper, malformed parser/upload bounds, MCP hostile inputs,
human-review gate regression, real process kill/resume, exclusive live retry, incremental
no-full-rerun plus watcher same-bytes retry, PostgreSQL concurrency, and Aurora/Harbor cross-corpus
denial. Injection detection is corpus-agnostic and contextual; it applies NFKC compatibility
folding only and does not claim generic homoglyph/confusable resistance. Local measurements
are one recorded sample in `docs/measurements/phase-09-local.json`, generated by the
executable measurement writer, with deterministic external cost `$0`. They are not
production SLAs.
